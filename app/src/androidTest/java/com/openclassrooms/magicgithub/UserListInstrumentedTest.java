...
public class UserListInstrumentedTest {
    ...
    public void checkIfRecyclerViewIsNotEmpty() {
        onView(withId(R.id.activity_list_user_rv)).check(new RecyclerViewUtils.ItemCount(currentUsersSize));//vérifie que la liste d'items android est de taille de la liste de user et donc pas vide.
    }

    @Test
    public void checkIfAddingRandomUserIsWorking() {
        onView(withId(R.id.activity_list_user_fab)).perform(click()); //clique sur le bouton d'ajout de l'app
        onView(withId(R.id.activity_list_user_rv)).check(new RecyclerViewUtils.ItemCount(currentUsersSize + 1)); //vérifie que la liste s'agrandit bien de 1.
    }
